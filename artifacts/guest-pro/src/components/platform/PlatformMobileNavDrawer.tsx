/**
 * PlatformMobileNavDrawer — left slide-in menu (mobile platform dashboard).
 */
...
import type { PlatformDashboardNavItem } from "@/lib/platform-dashboard-nav";

interface PlatformMobileNavDrawerProps {
  open: boolean;
  onClose: () => void;
  items: PlatformDashboardNavItem[];
  activeTab: PlatformDashboardNavItem["id"];
  appName: string;
  roleLine: string;
  onSelectTab: (tab: PlatformDashboardNavItem["id"]) => void;
}

export function PlatformMobileNavDrawer({
  open,
  onClose,
  items,
  activeTab,
        ...
        <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3.5">
          <div className="flex min-w-0 items-center gap-2.5">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/10">
              <GuestProLogo variant="header" className="h-5 w-5" />
            </div>
