 ...
 *
 * Employee-number departments: 4-digit number only (staff portal login).
 * Reception / Restaurant: email + password (not employee numbers).
 */
...
function buildCreateSchema(t: StaffTranslations) {
  return z
    ...
    .superRefine((data, ctx) => {
      if (isEmailLoginPersonnelDepartment(data.staffDepartment)) {
        const emailResult = z.string().email(t.staffEmailInvalid).safeParse(data.email ?? "");
        if (!emailResult.success) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: t.staffEmailInvalid,
            path: ["email"],
          });
        }
        if (!data.password || data.password.length < 8) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: t.staffPasswordMin,
            path: ["password"],
          });
        }
      } else if (!data.employeeNumber || !/^\d{4}$/.test(data.employeeNumber)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: t.staffEmployeeNumberInvalid,
          path: ["employeeNumber"],
        });
      }
    });
...
function toCreatePayload(
  ...
  const staffDepartment = lockedDepartment ?? values.staffDepartment;

  if (isEmailLoginPersonnelDepartment(staffDepartment)) {
    return {
...
export function CreateStaffSheet({
  ...
  const selectedDepartment = lockedDepartment ?? form.watch("staffDepartment");
  const usesEmailLogin = isEmailLoginPersonnelDepartment(selectedDepartment);
  ...
  const subtitle = usesEmailLogin ? t.addStaffSubtitleEmail : t.addStaffSubtitleNumber;
        ...
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit((v) =>
              mutation.mutate(toCreatePayload(v, lockedDepartment)),
            )}
            ...
            <div className="flex-1 space-y-3.5 overflow-y-auto overscroll-contain px-5 py-4">
              ...
              {usesEmailLogin ? (
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className={labelClass}>{t.staffEmployeeNumber}</FormLabel>
                      <FormControl>
                        <div className="relative">
