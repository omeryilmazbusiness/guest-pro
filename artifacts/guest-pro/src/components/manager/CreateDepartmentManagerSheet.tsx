/**
 * CreateDepartmentManagerSheet — GM-only flow to add a department manager account.
 */
...
const DEPT_ICONS: Record<DepartmentManagerDepartment, LucideIcon> = {
...
export interface CreateDepartmentManagerSheetProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function CreateDepartmentManagerSheet({
  open,
  onClose,
  onSuccess,
}: CreateDepartmentManagerSheetProps) {
  ...
  const mutation = useMutation({
    ...
    onError: (err: Error) => toast.error(err.message ?? "Failed to create department manager"),
  });
  ...
  return (
    <ManagerCenterSheet
      open={open}
      onClose={handleClose}
      ariaLabel="Add department manager"
      closeLabel={t.cancel}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 pt-0.5">
              ...
              <p className="mt-0.5 text-[12px] leading-snug text-zinc-500">
                Creates email + password login for a department manager.
              </p>
            </div>
          </div>
        </div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Sarah" autoComplete="given-name" className={fieldClass} {...field} />
                      </FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Yılmaz" autoComplete="family-name" className={fieldClass} {...field} />
                      </FormControl>
                      <FormMessage className="text-[11px]" />
                    </FormItem>
                  )}
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <div className="grid grid-cols-2 gap-1.5">
                        {DEPARTMENT_MANAGER_DEPARTMENTS.map((dept) => {
